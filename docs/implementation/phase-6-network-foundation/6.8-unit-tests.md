...
### 6. Play Mode Verifikation

Manuelle Verifikation im Unity Editor (nicht automatisierbar):

**Test-Checkliste:**
1. Szene `NetworkTestScene` öffnen
2. Play drücken, "Host" klicken
3. Zweite Unity-Instanz (ParrelSync) starten, "Client" klicken
4. Host bewegt sich → Client sieht Bewegung
5. Client bewegt sich → Host sieht Bewegung
6. Latenz simulieren (Tugboat hat ein "Latency Simulation" Feature)
7. Bei 100ms künstlicher Latenz: Lokale Bewegung bleibt flüssig
8. Bei Packet Loss: Remote-Spieler extrapoliert kurz, dann korrigiert
9. Ohne Netzwerk (kein Host/Client): Offline-Modus funktioniert wie bisher
